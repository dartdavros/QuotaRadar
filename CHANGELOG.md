...
- доверенные Codex-источники `@thsottiaux` и резервный `@sama`, создаваемые выключенными до настройки оператором;
- `quota_event_classifier v2` для строгой классификации массовых событий из корпоративных и персональных X-аккаунтов;
- ADR-0003 с политикой доверенных источников Codex;
- ручной ограниченный исторический импорт старых X-постов из Django Admin без сброса рабочего курсора;
- настройка количества постов за один исторический импорт;
- исходная дата публикации в Telegram-сообщении с настраиваемым часовым поясом;
- массовое возвращение ошибочных Telegram-доставок в очередь через Django Admin с обнулением счётчика попыток;

### Изменено

- первичная загрузка X-источника ограничена 10 последними постами и одной страницей;
- последующий polling использует `since_id`, страницы по 5 постов и получает все страницы только с новыми публикациями;
