...
Реализовать open-source систему QuotaRadar, которая автоматически отслеживает публикации утверждённых X-источников OpenAI и Anthropic, определяет через ИИ сообщения о сбросе, повышении или продлении повышенной квоты Codex/Claude Code и отправляет русскоязычные уведомления со ссылкой на источник в Telegram.

## 2. Термины

- **Источник** — X-аккаунт из утверждённого allowlist: `@OpenAIDevs`, `@thsottiaux`, `@sama` или `@ClaudeDevs`.
...
Система должна периодически проверять включённые записи утверждённого allowlist:

- `OpenAIDevs` — OpenAI/Codex, включён по умолчанию;
- `thsottiaux` — OpenAI/Codex, создаётся выключенным;
- `sama` — OpenAI/Codex, резервный, создаётся выключенным;
- `ClaudeDevs` — Anthropic/Claude Code, включён по умолчанию.

Список источников хранится в PostgreSQL, создаётся миграциями и управляется через Django Admin только в части включения и отключения. Добавление аккаунта вне allowlist требует отдельного ADR и миграции.
...
```http
GET https://api.x.com/2/users/by?usernames=<enabled_source_usernames>
...
### FR-003.1. Ручной исторический импорт

- Исторический импорт запускается оператором из списка источников Django Admin.
- Импорт использует самый старый сохранённый Post ID конкретного источника как границу и запрашивает только более старые публикации через `until_id`.
- Для исключения граничного дубликата в X API передаётся ID, непосредственно предшествующий самому старому сохранённому Post ID.
- Максимальное количество получаемых постов за один запуск задаётся `historical_backfill_post_limit`, по умолчанию — 100.
- Размер страницы X API не превышает 100; при необходимости используется пагинация.
- Ретвиты исключаются.
- `last_post_id`, `last_checked_at` и `last_success_at` обычного polling не изменяются.
- На ИИ-анализ ставятся только созданные в рамках текущего импорта `SourcePost`, от старых к новым.
- Повторный запуск продолжает импорт глубже истории от нового самого старого сохранённого поста.
- Ошибка исторического импорта не требует удаления постов, анализов, доставок или рабочего курсора.
...
- исходная дата публикации перед ссылкой на источник;
- дата преобразуется в часовой пояс `telegram_message_timezone`;
...
- массово возвращать доставки со статусом `failed` в очередь Telegram;
- запускать ограниченный исторический импорт для выбранных доверенных источников;
...
| `historical_backfill_post_limit` | int 5..3200 | Старых постов за один ручной импорт, по умолчанию 100 |
| `telegram_message_timezone` | IANA timezone | Часовой пояс даты в Telegram, по умолчанию `Europe/Moscow` |
...
```text
openai / OpenAIDevs / enabled=true
openai / thsottiaux / enabled=false
openai / sama / enabled=false
anthropic / ClaudeDevs / enabled=true
```
...
Фактическое количество Post reads и ИИ-анализов зависит от набора включённых источников. Включение `@thsottiaux` и особенно резервного `@sama` увеличивает объём нерелевантных публикаций, поэтому оператор должен контролировать стоимость X API и ИИ-провайдера.
...
5. X User ID всех включённых источников успешно разрешаются через X API.
