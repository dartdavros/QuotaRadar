...
QuotaRadar автоматически отслеживает новые публикации доверенных X-источников:

- `@OpenAIDevs` — официальный продуктовый источник Codex;
- `@thsottiaux` — основной дополнительный источник событий квот Codex;
- `@sama` — резервный источник массовых объявлений OpenAI;
- `@ClaudeDevs` — официальный продуктовый источник Claude Code.

Новые персональные источники создаются выключенными и включаются оператором после настройки первичной загрузки согласно ADR-0003.
...
Управляет утверждённым allowlist X-источников:
...
- утверждённый allowlist `@OpenAIDevs`, `@thsottiaux`, резервного `@sama` и `@ClaudeDevs`;
...
- политика доверенных источников согласно ADR-0003.

Не входят:

- RSS;
- scraping X;
- аккаунты вне утверждённого allowlist и другие продукты;
- ручная модерация;
