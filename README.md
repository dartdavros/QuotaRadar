...
QuotaRadar автоматически отслеживает публикации доверенных X-источников OpenAI и Anthropic, определяет массовые изменения квот Codex и Claude Code с помощью ИИ и отправляет уведомления на русском языке в Telegram. Для Codex поддерживаются `@OpenAIDevs`, `@thsottiaux` и резервный `@sama`; для Claude Code — `@ClaudeDevs`.
...
Система работает без ручной модерации. Каждый опубликованный Telegram-текст содержит исходную дату публикации и ссылку на пост в X.
...
- ручной безопасный импорт старых публикаций без сброса рабочего курсора;
...
Перед включением мониторинга откройте **Источники**:

- `@OpenAIDevs` и `@ClaudeDevs` уже активны;
- включите `@thsottiaux` как основной дополнительный источник Codex;
- оставьте `@sama` выключенным, если резервный источник не нужен;
- перед первым включением нового источника задайте **«Постов при первом опросе»**. Для загрузки последних 100 публикаций установите значение `100`.

Проверьте, что в **Системной конфигурации QuotaRadar** выбран активный промпт `quota_event_classifier v2`, затем включите мониторинг. Источники вручную создавать не нужно: они добавляются миграцией и доступны в Admin только для включения или отключения.

Если Telegram-доставка завершилась постоянной ошибкой, исправьте токен, права бота или получателя, затем откройте **Доставки Telegram**, выберите ошибочные записи и выполните действие **«Вернуть выбранные ошибочные доставки в очередь»**. Счётчик попыток начнётся заново. Доставки со статусом `Отправлено` повторно поставить в очередь нельзя.

Чтобы дополнительно загрузить более старую историю, откройте **Источники**, выберите нужные активные доверенные аккаунты и выполните действие **«Подтянуть старые посты выбранных источников»**. Количество постов за один запуск задаётся полем **«Постов за исторический импорт»** в системной конфигурации. Импорт получает публикации старше самого старого сохранённого поста, не меняет `last_post_id` и ставит на анализ только новые записи. Повторный запуск продолжает загрузку вглубь истории.

Дата исходной публикации выводится перед ссылкой в часовом поясе из поля **«Часовой пояс даты в Telegram»**. По умолчанию используется `Europe/Moscow`.
