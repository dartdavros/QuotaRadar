...
- `Постов при первом опросе` — начальное значение `10`, допустимо от `5` до `100`;
- `Постов на страницу регулярного опроса` — начальное значение `5`, допустимо от `5` до `100`;
...
### Повторный анализ ошибочных постов

В Django Admin откройте **Посты источников**, выберите записи со статусом
**Ошибка** и выполните действие **Вернуть выбранные ошибочные посты в очередь
анализа**. Действие сразу меняет статус на **Поставлен в очередь** и отправляет
отдельную Celery-задачу для каждого выбранного поста.
